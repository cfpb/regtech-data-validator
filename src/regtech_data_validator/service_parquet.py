...
from regtech_data_validator.validator import validate_batch_csv, validate_batch, ValidationResults
...
def service_validate_parquet(bucket, file):
    file_paths = [path for path in file.split('/') if path]
    ...
    for validation_results in validate_batch(f"s3://{bucket}/{file}", {"lei": lei}, batch_size=batch_size):
        buffer = io.BytesIO()
