...
def validate(schema: pa.DataFrameSchema, submission_df: pl.LazyFrame, row_start: int) -> pl.DataFrame:
    """
    ...
    try:
        ...
        for schema_error in err.schema_errors:
            ...
            if schema_error.reason_code is SchemaErrorReason.COLUMN_NOT_IN_DATAFRAME:
                raise ValidationSchemaError(schema_error) from schema_error

            # CHECK_ERROR is thrown by pandera polars if the check itself has a coding error, NOT if the check data results in an error
            if not check or schema_error.reason_code is SchemaErrorReason.CHECK_ERROR:
                raise RuntimeError(
...
def validate_batch_csv(path, context: dict[str, str] | None = None, batch_size: int = 50000, batch_count: int = 1):
    ...
    if not has_syntax_errors:
        ...
        findings = validate(register_schema, uids, 0)
        if not findings.is_empty():
            rf = format_findings(
...
def validate_chunks(schema, path, batch_size, batch_count):
    ...
    while batches:
        ...
        findings = validate(schema, df, row_start)
        batches = reader.next_batches(batch_count)
        row_start += df.height
        yield findings
